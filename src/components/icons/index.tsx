/**
 * Production icon set — the approved C3 voice: stroke 1.6, round caps and joins.
 *
 * These are the things a Pattaya renter actually asks about, drawn for this business.
 * Lucide is permitted ONLY inside borrowed primitives (the command palette); it must
 * never become the product's icon identity. See .altx/design-profile.yaml → iconography.
 */
type P = React.SVGProps<SVGSVGElement> & { size?: number; sw?: number };

const base = (size = 20, sw = 1.6): React.SVGProps<SVGSVGElement> => ({
  width: size,
  height: size,
  viewBox: "0 0 24 24",
  fill: "none",
...
/** Transport category marks — the catalogue must not borrow scooter semantics for cars. */
export const Car = ({ size, sw, ...p }: P) => (
  <svg {...base(size, sw)} {...p}>
    <path d="m5 16 1.6-5.3A2.5 2.5 0 0 1 9 9h6a2.5 2.5 0 0 1 2.4 1.7L19 16" />
    <path d="M3.5 16h17v3.5a1.5 1.5 0 0 1-1.5 1.5h-1.5v-2H6.5v2H5a1.5 1.5 0 0 1-1.5-1.5z" />
    <circle cx="7" cy="16" r="1" />
    <circle cx="17" cy="16" r="1" />
  </svg>
);

export const Shield = ({ size, sw, ...p }: P) => (
  <svg {...base(size, sw)} {...p}>
...
/** Navigation marks use the same quiet, round-stroked C3 voice as product icons. */
export const Menu = ({ size, sw, ...p }: P) => (
  <svg {...base(size, sw)} {...p}>
    <path d="M4 7h16M4 12h16M4 17h16" />
  </svg>
);

export const Close = ({ size, sw, ...p }: P) => (
  <svg {...base(size, sw)} {...p}>
